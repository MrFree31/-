...
int get_pixel(IMG *glif, int i, int j){//Переписал функцию проверки пикселей для лучшей читаемости и подручности. 
...
void diametr(IMG *glif){//Манхеттенская метрика
	...
	if(flag == 0){
		...
		int np = p_max - p_min;
		int nq = q_max - q_min;
		glif->diam = (np>nq)?np:nq;
	}
}

void svyaznost(IMG *glif){//Метод обхода в ширину графом.
	int *obhod = calloc(glif->w*glif->h,sizeof(int));
	int elements = 0;
	...
	for(int i = 0; i < glif->w; i++){
		for(int j = 0; j < glif->h; j++){
			...
			if(get_pixel(glif, i, j) && !obhod[id]){
				...
				obhod[id] = 1;
				while(vert<edge){
					int nast_i = line[vert][0];
					int nast_j = line[vert][1];
					vert++;
					for(int k = 0; k<4; k++){
						int new_i = nast_i + steps_check_i[k];
						int new_j = nast_j + steps_check_j[k];
						if(new_i>=0 && new_i<glif->w && new_j>=0 && new_j<glif->h){
							int new_id = glif->w*new_j + new_i;
							if(get_pixel(glif,new_i,new_j) && !obhod[new_id]){
								obhod[new_id] = 1;
								line[edge][0]=new_i;
								line[edge][1]=new_j;
								edge++;
...
void perimetr(IMG *glif){//Считаем перимерт по шагам вокруг пикселей.
	int P = 0;
	for(int i = 0; i<glif->w; i++){
		for(int j = 0; j<glif->h; j++){
			if(get_pixel(glif, i, j)){
				if(i==0 || get_pixel(glif, i-1, j)==0) P++;
				if(i==glif->w-1 || get_pixel(glif, i+1, j)==0) P++;
